...
new = kB*T/h.*exp(-H./kB./T).*exp(Ent./8.314);

kao = PA*new(1); kado = new(2); kbo = new(3); kbdo = new(4); kro = new(5);
...
%% Set up arrays
KB = kbo*logspace(-3,-1,10); % kbo = rate constant, logspace = pressure of B
DORC = zeros(length(KB),5)'; r = zeros(1,length(KB)); A = r; B = r; sigma_app = r; TRC_A = r; TRC_B = r; AB = B; AO = A; BB = B; BO = B; AA = B; OO = B; O = B;
%% Loop
for w = 1:length(KB)
    disp(w)
    for m = 1:1
        k0 = [kao kado KB(w) kbdo kro];
        for j = 1:length(k0)
            K = zeros(2,length(k0));
            K(1,:) = k0; K(2,:) = k0;
            K(2,j) = k0(j)*(1+delta);
...
%                 end
            ...
            sigma_app(m,w) = sum(DORC(1:2,w))+2*sum(DORC(3:5,w));
            TRC_A(m,w) = -(DORC(2,w)+DORC(5,w));
...
zA_AA = kado*AA(1,:)./(KB.*AO(1,:));
zA_AB = kado*AB(1,:)./(KB.*BO(1,:));
zA_AO = kado*(AA(1,:)-AO(1,:))./(KB.*(AO(1,:)-OO(1,:)));
zA_OO = kado*AO(1,:)./(KB.*OO(1,:));
...
hold on
set(gca,'linewidth', 2,'fontsize',12,'fontname','Arial')
plot(log10(KB),sum(DORC,1),'k-','linewidth',1)
ylim([0 2]);
title('DORC check');
