...
KA = logspace(-1,1,30); % k_ads array
kado = 10^0; % k_des for A2
kro = 10^0; % k_r
kbo = 10^0; % k_ads for B
kbdo = 1; % k_des for B
kadiffo = 10^5; % k_diff for a
kbdiffo = 10^5; % k_diff for b
...
%% Arrays for results
S = zeros(length(KA),7)'; r = zeros(1,length(KA)); A = r; O = r; AO = A; AA = A; OO = A; AAA = A; AAO = A; AOA = A; AOO = A; OAO = A; OOO = A;
    ...
    for m = 1:1
        ...
        for j = 1:length(k0)
            ...
            for i = 1:2
                ...
                b_ads_aa = @(x) 0;
                b_ads_ab = @(x) kb*aoo(x);
                b_ads_bb = @(x) kb*(oo(x)+2*boo(x));
                b_ads_bbb = @(x) 2*kb*(boo(x)+bboo(x));
                b_ads_bba = @(x) kb*(aoo(x) + aboo(x));
                b_ads_abo = @(x) -kb.*aboo(x);
                b_ads_ooo = @(x) -2*kb.*(ooo(x)+oooo(x));
                b_ads_aoo = @(x) -kb.*(aoo(x) + aooo(x));
                b_ads_aoa = @(x) 0;
                
                % B desorption
                bbbb = @(x) bbb(x).*bbb(x)./bb(x);
                abbb = @(x) bbb(x).*bba(x)./bb(x);
                aobb = @(x) aob(x).*bbo(x)./bo(x);
                
                b_des_a = @(x) 0;
                b_des_b = @(x) -2*kbd*bb(x);
                b_des_aa = @(x) 0;
                b_des_ab = @(x) -kbd*bba(x);
                b_des_bb = @(x) -kbd*(bb(x)+2*bbb(x));
                b_des_bbb = @(x) -2*kbd*(bbb(x)+bbbb(x));
                b_des_bba = @(x) -kbd*(bba(x) - abbb(x));
                b_des_abo = @(x) -kbd.*abbb(x);
                b_des_ooo = @(x) 2*kbd*(bbo(x)+bboo(x));
                b_des_aoo = @(x) kbd*(bba(x) + aobb(x));
                b_des_aoa = @(x) 0;
                ...
                ooaab = @(x) aoo(x).*aao(x)./ao(x).*aab(x)./aa(x);
