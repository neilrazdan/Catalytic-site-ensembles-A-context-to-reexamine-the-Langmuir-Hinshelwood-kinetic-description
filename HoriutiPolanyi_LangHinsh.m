...
%% Arrays for results
S = zeros(length(KA),5)'; r = zeros(1,length(KA)); A = r; O = r; sigma_app = r; AO = A; AA = A; OO = A; AAA = A; AAO = A; AOA = A; AOO = A; OAO = A; OOO = A;
    ...
    for m = 1:1
        ...
        for j = 1:length(k0)
            ...
            for i = 1:2
                ...
                % B adsorption
                b_ads_a = @(x) 0;
                b_ads_b = @(x) 2*kb*o(x).^2;
                
                
                % B desorption
                b_des_a = @(x) 0;
                b_des_b = @(x) -2*kbd*b(x).^2;
